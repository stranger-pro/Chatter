...
const Signin = () => {
  ...
  const {setUser,setIsAuth,isAuth} = useApp()
  const navigate = useNavigate()

  const submitHandler = async(event) => {
    try{
        event.preventDefault();
        const {data} = await axios.post(`${import.meta.env.VITE_SERVER_URL}/user/signin`,{
            email,
            password
        });
        if(data.success){
    ...
    {/* card */}
      ...
      {/* text */}
        <div className='flex  items-center flex-col'>
          ...
          <p className='text-sm text-gray-400 font-medium'>Signin to continue to chatter</p>
        </div>
